...
class MarkovChainJson:
    ...
    def iterate_sequences(self, initial_state=None, max_len=10, pad=False,
        ...
        for possible_next, val in self.records[current_state].items():
            ...
            if possible_next == self.END_OF_SEQ:
                score = self.get_sequence_score(seq, strategy)
                if score >= thresh:
                    yield seq, score
    ...
    # metrics
    def calc_removal_score(self, state, sequences=None, required_states=None,
                           blacklisted_states=None, max_seqs=100, *args,
                           **kwargs):
...
class MarkovCharJson(MarkovChainJson):
    def tokenize(self, text, wildcards=False):
        sequence = list(text)
        if wildcards:
...
class MarkovWordJson(MarkovChainJson):
    def generate_sequence(self, *args, **kwargs):
        seq = super().generate_sequence(*args, **kwargs)
        return " ".join([s for s in seq if
                         s != self.START_OF_SEQ and s != self.END_OF_SEQ])
